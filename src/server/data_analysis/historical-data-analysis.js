...
histController.readLastHour = (req, res, next) => {
  ...
  if(service !== 'aggregate'){
    ...
    queryText = `SELECT timestamp, service, method, availability, response_time, error_rate, load FROM ${THREE_MIN_TABLE} WHERE timestamp >= ${Date.now() - 360000}::BIGINT AND service = '${service}' AND method != 'aggregate';`;
    ...
    queryText = `SELECT timestamp, service, method, availability, response_time, error_rate, load FROM ${THREE_MIN_TABLE} WHERE timestamp >= ${Date.now() - 360000}::BIGINT AND method = 'aggregate';`;
